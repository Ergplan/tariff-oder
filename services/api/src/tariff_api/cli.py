...
def cmd_arr_schema(args: argparse.Namespace) -> int:
    """JSON schema of the ARR taxonomy and mapping files, generated from the models."""
    from .arr.taxonomy import Mapping, Taxonomy

    schema = {
        "$schema": "https://json-schema.org/draft/2020-12/schema",
        "title": "ARR taxonomy and commission mapping files",
        "description": "Generated by `tariff-api arr-schema` from tariff_api.arr.taxonomy; a unit test fails on drift.",
        "oneOf": [Taxonomy.model_json_schema(), Mapping.model_json_schema()],
    }
...
def cmd_arr_scan(args: argparse.Namespace) -> int:
    """Hand-mapping scanner: place every table-like line of an order's text on the ARR
    taxonomy and list what stays unplaced.  From an exchange folder (files only) or from
    a registered source (the PDF is read from the object store)."""
    from .arr.scan import scan_exchange_folder, scan_texts

    if args.exchange:
        folder = Path(args.exchange)
        rep = scan_exchange_folder(folder, commission=args.commission, licensee_kind=args.licensee_kind)
        out_path = Path(args.out) if args.out else folder / "arr-scan.json"
    else:
        if not args.source_id:
            print("give a source id or --exchange <folder>", file=sys.stderr)
            return 2
        ...
        storage: ObjectStore = adapters.storage
        with session_scope() as s:
            src = s.get(SourceDocument, uuid.UUID(args.source_id))
            if src is None:
                print("no such source", file=sys.stderr)
                return 1
            u = src.utility
            commission = args.commission or (u.commission.code if u is not None and u.commission else None)
            kind = args.licensee_kind or (u.licensee_kind if u is not None else "distribution")
            doc = open_document(storage.get(ObjectStore.SOURCES, src.object_key))
            try:
                texts = {i + 1: doc[i].get_text("text", sort=True) for i in range(doc.page_count)}
            finally:
                doc.close()
    ...
    if out_path:
        ...
        print(
            f"{d['pages_scanned']} pages, {d['rows_seen']} table-like rows: {d['placed_rows']} placed on "
            f"{len(d['placed'])} line items, {d['unplaced_rows']} unplaced ({len(d['unplaced'])} distinct labels), "
            f"{len(d['ambiguous'])} ambiguous; written to {out_path}"
        )
    else:
        print(text)
    return 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    p = sub.add_parser("arr-schema", help="export the ARR taxonomy/mapping JSON schema")
    p.add_argument("-o", "--output", default=None)
    p.set_defaults(fn=cmd_arr_schema)

    p = sub.add_parser("arr-scan", help="ARR hand-mapping scanner: place an order's table rows on the taxonomy")
    p.add_argument("source_id", nargs="?", default=None, help="a registered source (reads the PDF from the store)")
    p.add_argument("--exchange", default=None, help="an exported order folder (exchange/<COMMISSION>/<order>/)")
